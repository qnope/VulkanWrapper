...
//   #include "random.glsl"
//
// The Xi buffer should contain HemisphereSamples (4096 vec2 values in [0,1)).
...
// Constants
const uint HEMISPHERE_SAMPLE_COUNT = 4096;
...
layout(set = 0, binding = RANDOM_XI_BUFFER_BINDING, std430) readonly buffer HemisphereSamplesBuffer {
    vec2 xi_samples[HEMISPHERE_SAMPLE_COUNT];
};
...
vec2 get_sample(uint sample_index, ivec2 pixel) {
    uint idx = sample_index % HEMISPHERE_SAMPLE_COUNT;
    vec2 xi = xi_samples[idx];
    vec2 noise_uv = (vec2(pixel) + 0.5) / float(NOISE_TEXTURE_SIZE);
