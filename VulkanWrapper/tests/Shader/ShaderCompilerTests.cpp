...
// Custom include resolver test
TEST(ShaderCompilerTest, CustomIncludeResolver) {
...
void main() {
    ...
    vw::ShaderCompiler compiler;
    compiler.set_include_resolver(
        [](std::string_view includePath, const std::filesystem::path &, bool) {
            if (includePath == "virtual_header.glsl") {
                return std::make_optional(
                    std::make_pair(std::filesystem::path("virtual_header.glsl"),
                                   std::string("#define CUSTOM_VALUE 0.75\n")));
            }
            return std::optional<std::pair<std::filesystem::path, std::string>>{};
        });
