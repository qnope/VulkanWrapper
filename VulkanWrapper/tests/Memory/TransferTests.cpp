...
TEST(TransferTest, CopyBuffer) {
    ...
    vw::Transfer transfer;
    transfer.copyBuffer(cmd, srcBuffer.handle(), dstBuffer.handle(), 0, 0,
    ...
    for (size_t i = 0; i < testData.size(); ++i) {
        EXPECT_FLOAT_EQ(retrieved[i], testData[i]) << "Mismatch at index " << i;
    }
}
...
TEST(TransferTest, CopyBufferToImage) {
    ...
    auto stagingBuffer =
        vw::create_buffer<StagingBuffer>(*gpu.allocator, bufferSize);
...
TEST(TransferTest, CopyImageToBuffer) {
    ...
    for (size_t i = 0; i < sourcePixels.size(); ++i) {
        EXPECT_EQ(retrieved[i], sourcePixels[i]) << "Mismatch at byte " << i;
    }
}
...
TEST(TransferTest, BlitImage) {
    ...
    using StagingBuffer = vw::Buffer<std::byte, true, vw::StagingBufferUsage>;
    auto stagingBuffer =
        vw::create_buffer<StagingBuffer>(*gpu.allocator, srcBufferSize);
...
TEST(TransferTest, CopyImageToBufferAndSaveToDisk) {
    ...
    for (size_t y = 0; y < height; ++y) {
        for (size_t x = 0; x < width; ++x) {
            size_t i = y * width + x;
            sourcePixels[i * 4 + 0] =
                static_cast<std::byte>((x * 255) / width); // R gradient
            sourcePixels[i * 4 + 1] =
                static_cast<std::byte>((y * 255) / height);        // G gradient
    ...
    std::filesystem::path tempPath =
        std::filesystem::temp_directory_path() / "transfer_test_output.png";
...
TEST(TransferTest, MultipleTransferOperations) {
    ...
    vw::Transfer transfer;
    transfer.copyBuffer(cmd, buffer1.handle(), buffer2.handle(), 0, 0,
                        data1.size() * sizeof(float));
    transfer.copyBuffer(cmd, buffer1.handle(), buffer3.handle(), 0, 0,
                        data1.size() * sizeof(float));
    ...
    for (size_t i = 0; i < data1.size(); ++i) {
        EXPECT_FLOAT_EQ(retrieved2[i], data1[i])
            << "Buffer2 mismatch at index " << i;
        EXPECT_FLOAT_EQ(retrieved3[i], data1[i])
