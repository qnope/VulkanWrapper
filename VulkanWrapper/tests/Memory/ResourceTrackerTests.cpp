...
protected:
    ...
    const std::vector<vk::BufferMemoryBarrier2>& getPendingBufferBarriers() const {
        return tracker.m_pending_buffer_barriers;
    }

    const std::vector<vk::ImageMemoryBarrier2>& getPendingImageBarriers() const {
    ...
    std::vector<BufferStateInfo> getBufferStates(vk::Buffer buffer) {
        ...
        if (tracker.m_buffer_states.find(buffer) == tracker.m_buffer_states.end()) {
            return result;
        }
        ...
        for (const auto& stateSet : stateSets) {
            for (const auto& interval : stateSet.intervals.intervals()) {
                result.push_back({interval, stateSet.state.stage, stateSet.state.access});
            }
        }
        // Sort by offset for easier testing
        std::sort(result.begin(), result.end(), [](const BufferStateInfo& a, const BufferStateInfo& b) {
    ...
    std::vector<ImageStateInfo> getImageStates(vk::Image image) {
        ...
        if (tracker.m_image_states.find(image) == tracker.m_image_states.end()) {
            return result;
        }
        ...
        for (const auto& stateSet : stateSets) {
            for (const auto& interval : stateSet.intervals.intervals()) {
                result.push_back({interval, stateSet.state.layout, stateSet.state.stage, stateSet.state.access});
            }
        }
        // No simple sort for multi-dimensional image intervals, but we can rely on insertion order or just check existence
        return result;
    }
...
TEST_F(ResourceTrackerTest, Buffer_UntrackedRequest_GeneratesFullBarrier) {
    ...
    EXPECT_EQ(barriers[0].srcStageMask, vk::PipelineStageFlagBits2::eAllCommands);
    EXPECT_EQ(barriers[0].srcAccessMask, vk::AccessFlagBits2::eMemoryWrite | vk::AccessFlagBits2::eMemoryRead);
...
TEST_F(ResourceTrackerTest, Buffer_RAR_NoBarrier) {
    ...
    // State should be updated (or merged/kept? Actually request updates state to new usage)
    // Wait, if we request new state, the tracker updates the state to the NEW usage.
    // So subsequent barriers will transition FROM this new state.
    auto states = getBufferStates(buffer);
    ASSERT_EQ(states.size(), 1);
...
TEST_F(ResourceTrackerTest, Buffer_RAW_GeneratesBarrier) {
    ...
    EXPECT_EQ(barriers[0].dstStageMask, vk::PipelineStageFlagBits2::eVertexShader);
...
TEST_F(ResourceTrackerTest, Image_UntrackedRequest_AddsState) {
    ...
    EXPECT_EQ(barriers[0].dstStageMask, vk::PipelineStageFlagBits2::eColorAttachmentOutput);
    EXPECT_EQ(barriers[0].dstAccessMask, vk::AccessFlagBits2::eColorAttachmentWrite);
...
TEST_F(ResourceTrackerTest, Image_SameLayout_DifferentAccess_GeneratesBarrier) {
    ...
    tracker.request(ImageState{
        .image = image,
        .subresourceRange = range,
        .layout = vk::ImageLayout::eColorAttachmentOptimal, // Keeping same layout for read (e.g. input attachment)
        .stage = vk::PipelineStageFlagBits2::eFragmentShader,
    ...
    EXPECT_EQ(barriers[0].newLayout, vk::ImageLayout::eColorAttachmentOptimal);
    EXPECT_EQ(barriers[0].srcAccessMask, vk::AccessFlagBits2::eColorAttachmentWrite);
    EXPECT_EQ(barriers[0].dstAccessMask, vk::AccessFlagBits2::eInputAttachmentRead);
}

TEST_F(ResourceTrackerTest, Image_SubresourceOverlap_SplitsState) {
    vk::Image image = vk::Image(reinterpret_cast<VkImage>(0x200));
    // Range: Mip 0-2
    vk::ImageSubresourceRange fullRange{vk::ImageAspectFlagBits::eColor, 0, 3, 0, 1};
    ...
    vk::ImageSubresourceRange mip1Range{vk::ImageAspectFlagBits::eColor, 1, 1, 0, 1};
    tracker.request(ImageState{
        .image = image,
    ...
    // Mip 2: TransferDst
    auto states = getImageStates(image);
    // Note: IntervalSet might merge Mip 0 and Mip 2 if they are not adjacent in the set logic? 
    // Actually Mip 0 and Mip 2 are NOT adjacent (Mip 1 is in between). 
    // So we expect 3 intervals if we iterate them.
    // However, they share the same state object in our implementation (vector<pair<IntervalSet, State>>).
    // So we might have 2 entries in the vector:
    ...
    for (const auto& s : states) {
        if (s.interval.range.baseMipLevel == 0 && s.interval.range.levelCount == 1 && s.layout == vk::ImageLayout::eTransferDstOptimal) hasMip0 = true;
        if (s.interval.range.baseMipLevel == 1 && s.interval.range.levelCount == 1 && s.layout == vk::ImageLayout::eShaderReadOnlyOptimal) hasMip1 = true;
        if (s.interval.range.baseMipLevel == 2 && s.interval.range.levelCount == 1 && s.layout == vk::ImageLayout::eTransferDstOptimal) hasMip2 = true;
    }
...
TEST_F(ResourceTrackerTest, Image_PartialUntracked_GeneratesBarriers) {
    ...
    // Track Mip 0
    vk::ImageSubresourceRange mip0Range{vk::ImageAspectFlagBits::eColor, 0, 1, 0, 1};
    tracker.track(ImageState{
        .image = image,
    ...
    vk::ImageSubresourceRange requestRange{vk::ImageAspectFlagBits::eColor, 0, 2, 0, 1};
    tracker.request(ImageState{
        .image = image,
...
TEST_F(ResourceTrackerTest, Image_UntrackedArrayLayer_GeneratesBarrier) {
    ...
    vk::ImageSubresourceRange requestRange{vk::ImageAspectFlagBits::eColor, 0, 1, 2, 1};
    tracker.request(ImageState{
        .image = image,
        .subresourceRange = requestRange,
        .layout = vk::ImageLayout::eTransferDstOptimal,
...
TEST_F(ResourceTrackerTest, AS_UntrackedRequest_GeneratesFullBarrier) {
    vk::AccelerationStructureKHR as = vk::AccelerationStructureKHR(reinterpret_cast<VkAccelerationStructureKHR>(0x300));
    ...
    EXPECT_EQ(barriers[0].srcStageMask, vk::PipelineStageFlagBits2::eAllCommands);
    EXPECT_EQ(barriers[0].srcAccessMask, vk::AccessFlagBits2::eMemoryWrite | vk::AccessFlagBits2::eMemoryRead);
    EXPECT_EQ(barriers[0].dstStageMask, vk::PipelineStageFlagBits2::eAccelerationStructureBuildKHR);
    EXPECT_EQ(barriers[0].dstAccessMask, vk::AccessFlagBits2::eAccelerationStructureWriteKHR);
}
...
TEST_F(ResourceTrackerTest, AS_Build_GeneratesBarrier) {
    ...
    auto barriers = getPendingMemoryBarriers();
    ASSERT_EQ(barriers.size(), 1);
    EXPECT_EQ(barriers[0].srcStageMask, vk::PipelineStageFlagBits2::eAccelerationStructureBuildKHR);
    EXPECT_EQ(barriers[0].srcAccessMask, vk::AccessFlagBits2::eAccelerationStructureWriteKHR);
    EXPECT_EQ(barriers[0].dstStageMask, vk::PipelineStageFlagBits2::eRayTracingShaderKHR);
    EXPECT_EQ(barriers[0].dstAccessMask, vk::AccessFlagBits2::eAccelerationStructureReadKHR);
}
