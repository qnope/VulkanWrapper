...
ToneMappingPass::ToneMappingPass(
    ...
    vk::Format output_format, bool indirect_enabled)
    : ScreenSpacePass(device, allocator)
    , m_output_format(output_format)
    , m_indirect_enabled(indirect_enabled)
...
std::vector<Slot> ToneMappingPass::input_slots() const {
    if (m_indirect_enabled) {
        return {Slot::Sky, Slot::DirectLight,
                Slot::IndirectLight};
    }
    return {Slot::Sky, Slot::DirectLight};
}
...
void ToneMappingPass::execute(vk::CommandBuffer cmd,
    ...
    auto &cached = get_or_create_image(
        Slot::ToneMapped, width, height, frame_index,
        m_output_format,
    ...
    auto sky_view = get_input(Slot::Sky).view;
    auto direct_light_view = get_input(Slot::DirectLight).view;

    // IndirectLight is optional
    std::shared_ptr<const ImageView> indirect_view;
    if (m_indirect_enabled) {
        try {
            indirect_view = get_input(Slot::IndirectLight).view;
        } catch (...) {
            // Not wired -- use fallback
        }
    }

    execute_to_view(cmd, tracker, cached.view, sky_view,
                    direct_light_view, indirect_view,
                    m_indirect_intensity, m_current_operator,
                    m_exposure, m_white_point,
                    m_luminance_scale);
}

void ToneMappingPass::execute_to_view(
...
ToneMappingPass::CompiledShaders ToneMappingPass::compile_shaders(
    const std::filesystem::path &shader_dir) {
...
DescriptorPool ToneMappingPass::create_descriptor_pool(
    CompiledShaders shaders) {
    // Create descriptor layout for HDR input textures
    m_descriptor_layout =
    ...
    m_pipeline = create_screen_space_pipeline(
        m_device, shaders.vertex, shaders.fragment,
        m_descriptor_layout, m_output_format,
        vk::Format::eUndefined, vk::CompareOp::eAlways,
        push_constants);
...
void ToneMappingPass::create_black_fallback_image() {
    // Create a 1x1 black image for when indirect_view is not
    // provided
    ...
    m_black_image_view =
        ImageViewBuilder(m_device, m_black_image)
            .setImageType(vk::ImageViewType::e2D)
            .build();

    // Clear the image to black (all zeros) using a transfer
    // command
