...
const std::string &Exception::message() const noexcept { return m_message; }

void Exception::build_what() const {
    m_what_cache = std::format(
        "[{}:{}] {}\n  {}", extract_filename(m_location.file_name()),
        m_location.line(), m_location.function_name(), m_message);
}
...
vk::Result VulkanException::result() const noexcept { return m_result; }
...
void VulkanException::build_what() const {
    m_what_cache =
        std::format("[{}:{}] {}\n  {}\n  {}",
                    extract_filename(m_location.file_name()), m_location.line(),
                    m_location.function_name(), m_message, m_result_string);
}
...
void SDLException::build_what() const {
    m_what_cache = std::format(
        "[{}:{}] {}\n  {}\n  {}", extract_filename(m_location.file_name()),
        m_location.line(), m_location.function_name(), m_message, m_sdl_error);
}
...
VkResult VMAException::result() const noexcept { return m_result; }
...
FileException::FileException(std::filesystem::path path, std::string context,
                             std::source_location location)
    : Exception(std::move(context), location)
...
void FileException::build_what() const {
    m_what_cache =
        std::format("[{}:{}] {}\n  {}\n  {}",
                    extract_filename(m_location.file_name()), m_location.line(),
                    m_location.function_name(), m_message, m_path.string());
}
...
void AssimpException::build_what() const {
    m_what_cache = std::format("[{}:{}] {}\n  {}\n  {}\n  {}",
                               extract_filename(m_location.file_name()),
                               m_location.line(), m_location.function_name(),
                               m_message, m_path.string(), m_assimp_error);
}
...
ShaderCompilationException::ShaderCompilationException(
    ...
    std::string compilation_log, std::source_location location)
...
const std::string &
ShaderCompilationException::compilation_log() const noexcept {
...
void ShaderCompilationException::build_what() const {
    m_what_cache =
        std::format("[{}:{}] {}\n  Shader: {} ({})\n  Log:\n{}",
                    extract_filename(m_location.file_name()), m_location.line(),
                    m_location.function_name(), m_shader_name,
                    vk::to_string(m_stage), m_compilation_log);
}

// LogicException factory methods

LogicException LogicException::out_of_range(std::string_view what_is_invalid,
                                            std::size_t value, std::size_t max,
                                            std::source_location location) {
    return LogicException(std::format("{} {} is out of range [0, {})",
                                      what_is_invalid, value, max),
                          location);
}
...
ValidationLayerException::type() const noexcept {
    return m_type;
}

const std::string &
ValidationLayerException::validation_message() const noexcept {
...
void ValidationLayerException::build_what() const {
    ...
    m_what_cache = std::format("[{}:{}] {}\n  [{}][{}] {}",
                               extract_filename(m_location.file_name()),
                               m_location.line(), m_location.function_name(),
                               severity_str, type_str, m_validation_message);
}

// SwapchainException

SwapchainException::SwapchainException(vk::Result result, std::string context,
                                       std::source_location location)
    : Exception(std::move(context), location)
    , m_result(result) {}

vk::Result SwapchainException::result() const noexcept { return m_result; }

void SwapchainException::build_what() const {
    m_what_cache = std::format("[{}:{}] {}\n  {}\n  {}",
                               extract_filename(m_location.file_name()),
                               m_location.line(), m_location.function_name(),
                               m_message, vk::to_string(m_result));
}
