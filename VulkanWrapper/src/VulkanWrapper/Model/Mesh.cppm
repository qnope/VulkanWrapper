...
/// Push constants for mesh rendering with buffer reference
/// materials.
...
class Mesh {
  public:
    ...
    /// Draw the mesh using push constants with buffer device
    /// address.
    ...
    /// Meshes with the same geometry_hash() share identical
    /// triangle data.
    [[nodiscard]] size_t geometry_hash() const noexcept;

    /// Equality operator comparing geometry identity (not
    /// materials). Two meshes are equal if they reference the same
    /// GPU geometry.
...
export template <> struct std::hash<vw::Model::Mesh> {
