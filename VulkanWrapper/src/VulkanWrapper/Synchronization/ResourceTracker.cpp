...
void ResourceTracker::request_image(vk::Image image,
    ...
    // Track parts of the requested interval that are not covered by existing states
    ...
    for (auto &stateSet : stateSets) {
        auto overlapping = stateSet.intervals.findOverlapping(requestedInterval);
        ...
        if (!overlapping.empty()) {
            ...
            for (const auto &overlap : overlapping) {
                // Intersect with requested interval to only barrier what's needed
                ...
                for (const auto& rem : remainingIntervals) {
                    ...
                    nextRemaining.insert(nextRemaining.end(), diff.begin(), diff.end());
                }
    ...
    // Generate barriers for untracked (remaining) intervals (Undefined -> New Layout)
    for (const auto& interval : remainingIntervals) {
        // Only generate barrier if we are transitioning to a specific layout
        // (Undefined -> Undefined doesn't need a barrier, but Undefined -> Something does)
...
void ResourceTracker::request_buffer(vk::Buffer buffer, vk::DeviceSize offset,
    ...
    // If no overlap found (untracked), assume undefined state and generate full barrier if needed
    // Actually, for buffers, untracked usually means we don't know the state.
    // User requested: "When a buffer is requested, if there is no entry for this buffer, we should assume full memory barrier."
    if (!foundOverlap) {
         // Full memory barrier for the requested range
         // We assume the previous state could be anything (Undefined is not quite right for buffers, but effectively we need to wait for everything)
         // However, we can't really put "Undefined" stage/access for buffers in a useful way that matches "Full Memory Barrier" logic 
         // without using AllCommands/AllRead/AllWrite.
         
         // Let's use AllCommands -> Target Stage, MemoryRead|Write -> Target Access
         // Or simply srcStage=AllCommands, srcAccess=MemoryWrite?
         
         vk::BufferMemoryBarrier2 barrier;
         barrier.srcStageMask = vk::PipelineStageFlagBits2::eAllCommands;
         barrier.srcAccessMask = vk::AccessFlagBits2::eMemoryWrite | vk::AccessFlagBits2::eMemoryRead; 
         // Wait, if we don't know previous state, we must assume worst case: it was written to by any stage.
