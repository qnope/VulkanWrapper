...
void Transfer::blit(vk::CommandBuffer cmd,
    ...
    vk::ImageSubresourceRange srcSubresource =
        srcSubresourceOpt.value_or(src->full_range());
    vk::ImageSubresourceRange dstSubresource =
        dstSubresourceOpt.value_or(dst->full_range());

    // Request source image in transfer src layout
    Barrier::ImageState srcState{.image = src->handle(),
    ...
    Barrier::ImageState dstState{.image = dst->handle(),
    ...
    blitRegion.srcOffsets[1] =
        vk::Offset3D{static_cast<int32_t>(src->extent3D().width),
    ...
    blitRegion.dstOffsets[1] =
        vk::Offset3D{static_cast<int32_t>(dst->extent3D().width),
                     static_cast<int32_t>(dst->extent3D().height),
                     static_cast<int32_t>(dst->extent3D().depth)};

    cmd.blitImage(src->handle(), vk::ImageLayout::eTransferSrcOptimal,
                  dst->handle(), vk::ImageLayout::eTransferDstOptimal,
                  blitRegion, filter);
}

void Transfer::copyBuffer(vk::CommandBuffer cmd, vk::Buffer src, vk::Buffer dst,
                          vk::DeviceSize srcOffset, vk::DeviceSize dstOffset,
                          vk::DeviceSize size) {

    // Request source buffer
    Barrier::BufferState srcState{.buffer = src,
                                  .offset = srcOffset,
                                  .size = size,
                                  .stage =
...
void Transfer::copyBufferToImage(
    ...
    vk::DeviceSize bufferSize = dst->extent3D().width * dst->extent3D().height *
                                dst->extent3D().depth *
    ...
    cmd.copyBufferToImage(src, dst->handle(),
...
void Transfer::copyImageToBuffer(
    ...
    // Calculate buffer size needed (simplified - assumes tightly packed)
    vk::DeviceSize bufferSize = src->extent3D().width * src->extent3D().height *
                                src->extent3D().depth *
                                4; // Assume 4 bytes per pixel
    ...
    cmd.copyImageToBuffer(src->handle(), vk::ImageLayout::eTransferSrcOptimal,
                          dst, copyRegion);
}
...
void Transfer::saveToFile(vk::CommandBuffer cmd, const Allocator &allocator,
                          Queue &queue,
    ...
    m_resourceTracker.request(
        Barrier::ImageState{.image = image->handle(),
