...
public:
    ...
    shaderc_include_result *GetInclude(const char *requested_source,
        ...
        if (m_custom_resolver) {
            auto result = m_custom_resolver(requested_source, requester_path, is_system);
            if (result) {
                m_included_files.insert(result->first);
                return make_include_result(result->first.string(), std::move(result->second));
            }
        }
...
private:
    ...
    const IncludeResolver &m_custom_resolver;
    std::unordered_set<std::filesystem::path> &m_included_files;
...
struct ShaderCompiler::Impl {
    ...
    IncludeResolver customResolver;
...
ShaderCompiler &ShaderCompiler::set_include_resolver(IncludeResolver resolver) {
    m_impl->customResolver = std::move(resolver);
    return *this;
}
...
ShaderCompilationResult ShaderCompiler::compile(std::string_view source,
    ...
    // Set includer
    options.SetIncluder(
        std::make_unique<VwIncluder>(m_impl->includePaths, m_impl->customResolver, included_files));
...
ShaderCompilationResult ShaderCompiler::compile_from_file(const std::filesystem::path &path,
    ...
    compiler_copy.m_impl->customResolver = m_impl->customResolver;
