...
constexpr std::size_t HEMISPHERE_SAMPLE_COUNT = 4096;
...
struct HemisphereSamples {
    std::array<glm::vec2, HEMISPHERE_SAMPLE_COUNT> samples;
};

/// Buffer type alias for hemisphere samples storage buffer
using HemisphereSamplesBuffer =
    Buffer<HemisphereSamples, true, StorageBufferUsage>;

/// Generate hemisphere samples using std::mt19937
/// @return HemisphereSamples structure with values in [0, 1)
[[nodiscard]] HemisphereSamples generate_hemisphere_samples();

/// Generate hemisphere samples using a specific seed for reproducibility
/// @param seed Random seed
/// @return HemisphereSamples structure with values in [0, 1)
[[nodiscard]] HemisphereSamples generate_hemisphere_samples(std::uint32_t seed);
...
[[nodiscard]] HemisphereSamplesBuffer
